...
time_range = Time([start_mjd, end_mjd], format='mjd', scale='utc')
alt_constraint = AltitudeConstraint(min=0 * u.deg) #Has to be in the sky 
alt_mask = is_observable(
    constraints=[alt_constraint],
    observer=chime,
    targets=[crab],
    time_range=time_range,
    time_grid_resolution=dt,
)[0]

n_steps = int(((time_range[1] - time_range[0]) / dt).decompose().value)
times = time_range[0] + np.arange(n_steps) * dt #Making array of all the times 
ha = chime.target_hour_angle(times, crab) #Calculate HA for each
...
total_obs_time = (time_range[1] - time_range[0]).to(u.hour)
